...
export default async function routes(app) {
  ...
  app.get('/trips/:tripId/links', { preHandler: app.requireOrganizer }, async (req) => {
  ...
  app.post('/links/:linkId/revoke', { preHandler: app.requireOrganizer }, async (req, reply) => {
    const link = app.db.prepare('SELECT id FROM participant_links WHERE id = ?').get(req.params.linkId)
